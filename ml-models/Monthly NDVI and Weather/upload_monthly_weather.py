...
LATITUDE  = sum(p[0] for p in RANGALA_POLYGON) / len(RANGALA_POLYGON)
LONGITUDE = sum(p[1] for p in RANGALA_POLYGON) / len(RANGALA_POLYGON)
...
def fetch_weather_from_openmeteo(year, month):
    start = f"{year}-{month:02d}-01"
    end   = f"{year+1}-01-01" if month == 12 else f"{year}-{month+1:02d}-01"

    print(f"Fetching weather from Open-Meteo: {start} → {end}")
    print(f"  lat={LATITUDE:.5f}  lon={LONGITUDE:.5f}")
    ...
    daily = daily[daily["date"].dt.month == month]

    avg_temperature = round(float(daily["temperature"].mean()), 4)
    avg_rainfall    = round(float(daily["rainfall"].mean()),    4)
    avg_humidity    = round(float(daily["humidity"].mean()),    4)

    print(f"  Days: {len(daily)}")
    print(f"  avg_temperature = {avg_temperature} °C")
    print(f"  avg_rainfall    = {avg_rainfall} mm")
    print(f"  avg_humidity    = {avg_humidity} %")

    return avg_temperature, avg_rainfall, avg_humidity


def upload_weather(year, month, avg_temperature, avg_rainfall, avg_humidity, div_map):
    month_str = MONTH_NAMES[month]
    updated = errors = 0

    print(f"\nPatching weather → division_ndvi_climate ({month_str} {year})...")
...
if __name__ == "__main__":
    print(f"\n{'='*55}")
    print(f"  Monthly Weather Upload — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"{'='*55}\n")
    ...
    avg_temperature, avg_rainfall, avg_humidity = fetch_weather_from_openmeteo(year, month)
    upload_weather(year, month, avg_temperature, avg_rainfall, avg_humidity, div_map)

    print(f"\n🎉 Weather for {MONTH_NAMES[month]} {year} saved to Supabase.\n")
